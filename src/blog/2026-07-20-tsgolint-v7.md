...
`typeAware` enables rules that require type information. The experimental `typeCheck` option additionally reports TypeScript compiler diagnostics; omit it if you only want type-aware linting. JSON config users can set the same options under `options` in `.oxlintrc.json`. Both options are root-only, and CLI flags take precedence.

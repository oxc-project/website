...
When `oxc.typeAware` is unset, each file follows the `options.typeAware` value of the config that governs it, so a single package can enable type-aware linting through its own [nested config](./nested-config). When set explicitly, it overrides every config and applies to every file.
